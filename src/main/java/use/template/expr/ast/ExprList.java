...
public class ExprList extends Expr {
  ...
  public Object eval(Scope scope, Env env) {
    ...
    if (exprArray.length == 1) {
      return exprArray[0].eval(scope, env);
    }
    ...
    for (int i = 0; i < end; i++) {
      exprArray[i].eval(scope, env);
    }
    return exprArray[end].eval(scope, env);
  }

  /**
   对所有表达式求值，并返回所有表达式的值
   */
  public Object[] evalExprList(Scope scope, Env env) {
    if (exprArray.length == 0) {
    ...
    for (int i = 0; i < exprArray.length; i++) {
      ret[i] = exprArray[i].eval(scope, env);
    }
    return ret;
  }
}
