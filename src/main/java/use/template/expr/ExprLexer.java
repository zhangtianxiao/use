...
import use.template.stat.ParaToken;
...
class ExprLexer {
  ...
  public ExprLexer(ParaToken paraToken, Location location) {
    ...
    String content = paraToken.getContent();
    ...
    for (char c = next(); true; c = next()) {
      if (c == quotes) {
        if (buf[forward - 1] != '\\') {  // 前一个字符不是转义字符
          String sb = subBuf(lexemeBegin + 1, forward - 1);
          String str;
          if (sb != null) {
            if (quotes == '"') {
  ...
  boolean scanNumber() {
    ...
    if (c == '.') {              // 以 '.' 字符结尾是合法的浮点数
      ...
      if (peek() == '.' ||        // 处理 [0..9] 这样的表达式
        ...
        String n = subBuf(numStart, forward - 2);
        if (n == null /* && radix == 16 */) {
        ...
        NumTok tok = new NumTok(Sym.INT, n, radix, false, location);
        addToken(tok);
        retract(1);
        return prepareNextScan();
      }
  ...
  String subBuf(int start, int end) {
    if (start > end) {
      return null;
    }
