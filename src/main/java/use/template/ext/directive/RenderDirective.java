...
import use.template.stat.ast.Include;
...
public class RenderDirective extends Directive {
  ...
  public void exec(Env env, Scope parent, Writer writer) {
    ...
    ObjectPool<Scope> scopeFactory = env.config.scopePool;
  ...
  private SubStat parseSubStat(Env env, String subFileName) {
    ...
    // FileSource subFileSource = new FileSource(config.getBaseTemplatePath(), subFileName, config.encoding);
    ISource subFileSource = config.sourceFactory.getSource(subFileName, config.encoding);

    try {
      SubEnv subEnv = new SubEnv(env);
      StatList subStatList = new Parser(subEnv, subFileName).parse(subFileSource);
