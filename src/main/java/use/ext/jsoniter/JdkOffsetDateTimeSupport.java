...
public class JdkOffsetDateTimeSupport {
  public static void main(String[] args) {
    ...
    for (int i = 0; i < 10000; i++) {
      bytes = OffsetDateTimeBytes(now, 0);
  ...
  public static byte[] OffsetDateTimeBytes(OffsetDateTime that, int quote) {
    if (quote == 0 | quote == 1) {
    }
  ...
  static final Encoder.ReflectionEncoder reflectionEncoder = new Encoder.ReflectionEncoder() {
    ...
    public void encode(Object obj, JsonStream stream) throws IOException {
      ...
      stream.write(OffsetDateTimeBytes((OffsetDateTime) obj, 1));
      //stream.writeVal(val);
    }

    @Override
    public Any wrap(Object obj) {
      byte[] bytes = OffsetDateTimeBytes((OffsetDateTime) obj, 1);
      return Any.lazyString(bytes, 0, bytes.length);
    }
  };

  public static void enable() {
    JsoniterSpi.registerTypeEncoder(OffsetDateTime.class, reflectionEncoder);
    JsoniterSpi.registerTypeDecoder(OffsetDateTime.class, iter -> {
      try {
        ...
        OffsetDateTime date = OffsetDateTime.of(year, month, day, hour, minute, second, 0, Helper.zoneOffset);
    ...
    JsonStream.registerNativeEncoder(OffsetDateTime.class, reflectionEncoder);
  }
}
